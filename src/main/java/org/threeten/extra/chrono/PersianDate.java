 ...
 * <p>
 * This implements {@code ChronoLocalDate} for the {@link PersianChronology Persian calendar}.
 *
 * <h3>Specification for implementors</h3>
 * This class is immutable and thread-safe.
 */
public final class PersianDate
        extends DefaultInterfaceChronoLocalDate<PersianDate>
        implements ChronoLocalDate<PersianDate>, Serializable {

    /**
     * Serialization version.
     */
	private static final long serialVersionUID = 3796406004087932495L;
    /**
     * The difference between the Persian and Persian epoch day count.
     */
    ...
    //-----------------------------------------------------------------------
    /**
     * Creates a date in Persian calendar system from the Era, year-of-era,
     * month-of-year and day-of-month.
     *
     * @param era  the PersianEra, not null
     ...
     */
    public static PersianDate of(PersianEra era, int year, int month, int dayOfMonth) {
        return (PersianDate)PersianChronology.INSTANCE.date(era, year, month, dayOfMonth);
    }
     ...
     * @return the Persian date, not null
     * @throws DateTimeException if the date is invalid
     */
    public static PersianDate ofEpochDay(long epochDay) {
        // TODO: validate
//        if (epochDay < MIN_EPOCH_DAY || epochDay > MAX_EPOCH_DAY) {
//            throw new CalendricalRuleException("Date exceeds supported range for PersianDate", Persian.YEAR);
//        }
        ...
        int month = doy0 / 30 + 1;
        int dom = doy0 % 30 + 1;
        return new PersianDate(prolepticYear, month, dom);
    }

    private static PersianDate resolvePreviousValid(int prolepticYear, int month, int day) {
        if (month == 13 && day > 5) {
            day = PersianChronology.INSTANCE.isLeapYear(prolepticYear) ? 6 : 5;
        }
        return new PersianDate(prolepticYear, month, day);
    }
     ...
     * Creates an instance.
     *
     * @param prolepticYear  the Persian proleptic-year
     * @param month  the Persian month, from 1 to 13
     * @param dayOfMonth  the Persian day-of-month, from 1 to 30
     * @throws DateTimeException if the date is invalid
     */
    PersianDate(int prolepticYear, int month, int dayOfMonth) {
        PersianChronology.MOY_RANGE.checkValidValue(month, MONTH_OF_YEAR);
        ValueRange range;
        if (month == 13) {
            range = PersianChronology.INSTANCE.isLeapYear(prolepticYear) ? PersianChronology.DOM_RANGE_LEAP : PersianChronology.DOM_RANGE_NONLEAP;
        } else {
            range = PersianChronology.DOM_RANGE;
        }
    ...
    //-------------------------------------------------------------------------
    @Override
    public PersianDate with(TemporalAdjuster adjuster) {
        return (PersianDate) adjuster.adjustInto(this);
    }

    @Override
    public PersianDate with(TemporalField field, long newValue) {
        if (field instanceof ChronoField) {
            ChronoField f = (ChronoField) field;
    ...
    //-----------------------------------------------------------------------
    @Override
    public PersianDate plus(TemporalAmount amount) {
        return (PersianDate) super.plus(amount);
    }

    @Override
    public PersianDate plus(long amountToAdd, TemporalUnit unit) {
...
//                case FOREVER: return (period == 0 ? this : (period > 0 ? LocalDate.MAX_DATE : LocalDate.MIN_DATE));
            ...
            throw new DateTimeException(unit.getName() + " not valid for PersianDate");
        }
        return unit.addTo(this, amountToAdd);
    }

    //-----------------------------------------------------------------------
    private PersianDate plusYears(long years) {
        return plusMonths(Jdk8Methods.safeMultiply(years, 13));
    }

    private PersianDate plusMonths(long months) {
        if (months == 0) {
            return this;
        }
    ...
    private PersianDate plusDays(long days) {
        if (days == 0) {
            return this;
        }
        return PersianDate.ofEpochDay(Jdk8Methods.safeAdd(toEpochDay(), days));
    }

    //-------------------------------------------------------------------------
    @Override
    public PersianDate minus(TemporalAmount amount) {
        return (PersianDate) super.minus(amount);
    }

    @Override
    public PersianDate minus(long amountToSubtract, TemporalUnit unit) {
        return (PersianDate) super.minus(amountToSubtract, unit);
    }

//    public PersianDate withEra(Era era) {
...
//    }
//
//    public PersianDate withDayOfMonth(int dayOfMonth) {
//        return with(ChronoField.DAY_OF_MONTH, month);
//    }
//
//    public PersianDate withDayOfYear(int dayOfYear) {
//        return with(ChronoField.DAY_OF_YEAR, month);
//    }
//
//    public PersianDate minusYears(long yearsToSubtract) {
//        return (yearsToSubtract == Long.MIN_VALUE ? plusYears(Long.MAX_VALUE).plusYears(1) : plusYears(-yearsToSubtract));
//    }
//
//    public PersianDate minusMonths(long monthsToSubtract) {
//        return (monthsToSubtract == Long.MIN_VALUE ? plusMonths(Long.MAX_VALUE).plusMonths(1) : plusMonths(-monthsToSubtract));
//    }
//
//    public PersianDate minusWeeks(long weeksToSubtract) {
//        return  (weeksToSubtract == Long.MIN_VALUE ? plusWeeks(Long.MAX_VALUE).plusWeeks(1) : plusWeeks(-weeksToSubtract));
//    }
//
//    public PersianDate minusDays(long daysToSubtract) {
...
//    }
    ...
    public long toEpochDay() {
        ...
        long persianEpochDay = ((year - 1) * 365) + Jdk8Methods.floorDiv(year, 4) + (getDayOfYear() - 1);
        return persianEpochDay - EPOCH_DAY_DIFFERENCE;
    }
